...
## It requires output from 'find_common_cpd_cell_pairs.R': combination_nonuniq_GSE70138.
## Note:
...
repeated_cpd_cell_proj_combs <- readRDS("path/to/combination_nonuniq_GSE70138.rds")
...
for (tmp_pert_id in all_pert_ids) {
  ...
  for (tmp_group_id in all_tmp_group_ids) {
    ...
    for (tmp_cell_id in all_tmp_cell_ids) {
      ## check if a combination of perturbation id, cell id, and group id are repeated
