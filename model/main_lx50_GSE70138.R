...
## This function runs potency/efficacy calculation for all combinations of compounds and cell lines.
## Note:
    ...
    for (tmp_cell_id in all_tmp_cell_ids) {
      ...
      if (length(unique(tmp_gct_df_pert$pert_dose)) >= 5) {
        ...
        if (class(all.fits)[1] == "gam") {
          mfc_lx50 <- tryCatch({
            do.call('rbind', get.gam.stats(all.fits, B = 10001))
          }, error = function(e) {
            e
          })
          ## if error with potency/efficacy calculation
          if (inherits(mfc_lx50, "error")) {
            all.errs <- data.frame(
              gene_id = gene_id,
              pert_id = tmp_pert_id,
              group_id = tmp_group_id,
              cell_id = tmp_cell_id,
              error = toString(mfc_lx50)
            )
            all_errs_lst[[k2]] <- all.errs
            k2 <- k2 + 1
            next
          }
          # if no error with potency/efficacy calculation
          if (class(mfc_lx50) == "data.frame") {
            ## add pert_id, cell_id, and group_id
            mfc_lx50 <- mfc_lx50 %>%
              remove_rownames() %>%
...
mfc_lx50_df <- do.call('rbind', mfc_lx50_lst)
all_errs_df <- do.call('rbind', all_errs_lst)

## convert ln_conc to log10_conc
mfc_lx50_df <- mfc_lx50_df %>% 
  mutate(lx50 = lx50 / log(10),
         SE.lx50 = SE.lx50 / log(10))
