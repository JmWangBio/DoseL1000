...
## This script fits mixed models and estimates log2 fold change for all combinations of compounds and cell lines.
## Note:
...
## 3. Make sure to update the paths to the cmapR package, funs folder, GSE70138_Broad_LINCS_gene_info_2017-03-06.txt.gz, GSE70138_Broad_LINCS_inst_info_2017-03-06.txt.gz,
## GSE70138_Broad_LINCS_pert_info_2017-03-06.txt.gz, and GSE70138_Broad_LINCS_Level3_INF_mlr12k_n345976x12328_2017-03-06.gctx. GSE70138_Broad_LINCS_Level3_INF_mlr12k_n345976x12328_2017-03-06.gctx.gz needs to be unzipped.
...
## load metadata
gene_info <- read.delim("path/to/GSE70138_Broad_LINCS_gene_info_2017-03-06.txt.gz",
                        sep = "\t")
inst_info <- read.delim("path/to/GSE70138_Broad_LINCS_inst_info_2017-03-06.txt.gz",
                        sep = "\t")
pert_info <- read.delim("path/to/GSE70138_Broad_LINCS_pert_info_2017-03-06.txt.gz",
                        sep = "\t")
...
## read data
gct_lv3 <- parse_gctx(fname="path/to/GSE70138_Broad_LINCS_Level3_INF_mlr12k_n345976x12328_2017-03-06.gctx",
                      rid = lm_rid_lst[i])

## build gct data with perturbation information integrated
gct_df <- gct_lv3@mat %>%
  as.data.frame(.) %>%
...
for (tmp_pert_id in all_pert_ids) {
  ...
  ## retrieve all unique perturbagen group IDs
  ...
  for (tmp_group_id in all_tmp_group_ids) {
    ...
    for (tmp_cell_id in all_tmp_cell_ids) {
      ...
      tmp_DMSO_inst_ids <- inst_info %>% 
        filter(det_plate %in% gct_df_pert$det_plate,
               pert_id == "DMSO", 
               cell_id == tmp_cell_id) %>%
        pull(inst_id)
      ...
      tmp_gct_df_pert <- gct_df_pert %>%
        ...
        dplyr::select(gene, inst_id, abundance, cell_id, 
                      pert_dose, pert_time, det_plate, pert_id) %>%
        mutate(pert_dose = ifelse(pert_id == "DMSO", 0, pert_dose),
               pert_time = factor(pert_time, 
                                  levels = unique(pert_time)))
      
      ## check if only one plate - if yes, skip to next
      if (length(unique(tmp_gct_df_pert$det_plate)) == 1) {
        next
      }
      ...
      ## ln-transform concentration; convert plate to factor
      tmp_gct_df_pert <- tmp_gct_df_pert %>% 
        mutate(logConc = log(pert_dose + min_conc),
               det_plate = factor(as.numeric(factor(det_plate))))
      ...
      if (length(unique(tmp_gct_df_pert$pert_dose)) >= 5) {
        ...
        if (class(all.fits)[1] == "gam") {
          all.tests <- tryCatch({
            get.gamer.log2fc(all.fits=all.fits,
                             d.grid=d.grid)
          }, error = function(e) {
            e
          })
          # if error with est. of log2fc
          ...
          # if no error with est. of log2fc
          if (class(all.tests) == "data.frame") {
            all.tests$model <- "gamer"
          }
        } else {
          next
        }
        ...
        if (length(unique(tmp_gct_df_pert$pert_time)) >= 2) {
          all.fits <- tryCatch({rlmer(abundance ~ logConc * pert_time + (1 | det_plate),
                                      data = tmp_gct_df_pert)}, error = function(e) {
                                        e
                                      })
        } else {
          all.fits <- tryCatch({rlmer(abundance ~ logConc + (1 | det_plate),
                                      data = tmp_gct_df_pert)}, error = function(e) {
                                        e
                                      })
        }
        ...
        ## if no error with model fitting
        if (class(all.fits)[1] == "rlmerMod") {
          all.tests <- tryCatch({
            get.rlmer.log2fc(all.fits=all.fits,
                             d.grid=d.grid)
          }, error = function(e) {
            e
          })
